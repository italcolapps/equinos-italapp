...
def authenticate_user(credentials):
    ...
    if not (credentials['user'] in users['usuario_'].values):
        return [False, None]
    authed = (credentials['user'] in users['usuario_'].values) and (credentials['password'] == (users.loc[users['usuario_'] == credentials['user'], 'contraseña'].values[0]))
    return [authed, users.loc[users['usuario_'] == credentials['user']][['nombre', 'usuario_', 'doc_id', 'rol_usuario', 'pais']]]
